...
SYSTEM = """Ты — ассистент студии {author} (FPV-съёмка недвижимости, Москва),
которая разослала агентствам КП о видеотурах. Агентство ответило на письмо.
Пиши от «мы», тон премиальный и сдержанный.
...
- полезные факты (если уместно): до 3–4 объектов за съёмочный день,
  готовый ролик через 1–2 дня, 8+ лет опыта пилота, ни одного инцидента
...
def check(agencies: list[dict], pending: dict, last_uid: int, cfg: dict, log) -> int:
    ...
    for uid in uids:
        ...
        try:    # =?utf-8?B?…?= → человеческий текст
